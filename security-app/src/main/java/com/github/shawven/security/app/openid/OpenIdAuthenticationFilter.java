...
public class OpenIdAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	private String openIdParameter = ConnectConstants.OPENID_PARAMETER_NAME;
	private String providerIdParameter = ConnectConstants.PROVIDER_ID_PARAMETER_NAME;
	private boolean postOnly = true;

	public OpenIdAuthenticationFilter() {
		super(new AntPathRequestMatcher(ConnectConstants.DEFAULT_OPENID_TOKEN_PROCESSING_URL, "POST"));
	}
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isBlank(openid)) {
            throw new IllegalArgumentException("参数错误，openId不能为空");
        }
        if (StringUtils.isBlank(providerId)) {
            throw new IllegalArgumentException("参数错误，providerId不能为空");
        }
